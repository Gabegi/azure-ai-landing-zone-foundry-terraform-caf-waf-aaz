...
az vm run-command invoke `
  ...
  --scripts "systemctl stop unattended-upgrades && apt-get update -y && apt-get install -y unzip curl wget git jq && curl -fsSL https://deb.nodesource.com/setup_20.x | bash - && apt-get install -y nodejs && curl -sL https://aka.ms/InstallAzureCLIDeb | bash && mkdir -p /home/azureuser/actions-runner && cd /home/azureuser/actions-runner && curl -o runner.tar.gz -L https://github.com/actions/runner/releases/download/v2.317.0/actions-runner-linux-x64-2.317.0.tar.gz && tar xzf runner.tar.gz && RUNNER_ALLOW_RUNASROOT=1 ./config.sh --url https://github.com/$GITHUB_ORG/$GITHUB_REPO --token $GITHUB_RUNNER_TOKEN --name $VM_NAME --unattended && sudo ./svc.sh install && sudo ./svc.sh start"
